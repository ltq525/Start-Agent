...
class GAIADataset:
    ...
    @staticmethod
    def _normalize_level(level: Any) -> Any:
        """Normalize GAIA levels because HuggingFace metadata stores them as strings."""
    ...
    def _standardize_item(self, item: Dict[str, Any]) -> Dict[str, Any]:
        """标准化数据项格式"""
        level = self._normalize_level(item.get("Level", item.get("level", 1)))
        if level is None:
            level = 1
